...
3. Open a terminal in this repo's root
4. Run `cargo build`
5. The built binaries may be found under `./target`
...
- `qmldiff create-hashtab <QML root> [output hashtab path]`
    * Creates a hashtab file from all the files within `QML root` recursively.
- `qmldiff hash-diffs <hashtab> <diff 1> [diff 2]... [-r]`
    ...
    * To unhash a diff, specify the `-r` flag.
- `qmldiff apply-diffs [--hashtab <hashtab>] <QML root> <QML destination> [...diffs] [-f] [-c]`
...
If the file you're trying to patch is not a QML file, use the `AFFECT REBUILD` instruction.

Example:
```
AFFECT REBUILD /data/example.json
    ; REBUILD instructions go here
    LOCATE { "foo" }
    REMOVE LOCATED
    INSERT { "bar" }
; Important: Use 'END REBUILD' here - not 'END AFFECT!'
END REBUILD
```
...
The load statement loads the file with the path given as a QMLDiff file.

#### `LOAD EXTERNAL <file_path>`

Loading of external files is delegated to a C function when running qmldiff as a library (if `LOAD EXTERNAL` is encountered and a C handler isn't set up, qmldiff will error). When using qmldiff as a standalone app, this statement will only log the file name of the file the QMD tried to load to stdout.
...
This statement displays the string provided in the development console of the application qmldiff is running in.

#### `ERROR <string>`

This statement is similar to `INFO`, but after displaying the string, it crashes the program.
...
Qmldiff supports conditional code. It is possible to check for:

- The existence of a given element in the tree
- The presence of another QMD file in the global list of files to apply
- Whether the system version matches a given string, either exactly or as a regex
- The existence of a given slot

In addition to that, it also supports boolean operations (NOT / OR / AND), else-if and else statements.
...
To check if some element in the tree exists, use the `EXISTS` command:

```
IF EXISTS Item > Object
    LOCATE BEFORE ALL
    ...
END IF
```
...
To check if a given slot has been written to, use the `SLOT` command:
...
To check if a diff file exists, use the `DIFF` command:
...
It is also possible to check whether the system version matches a certain value - either via a regex, or direct string matching:

```
; Regex:
IF VERSION MATCHES "myRegex"
    ...
END IF
```
...
As is the case in many other languages, one can place an `ELSE` or an `ELIF` statement after an `IF`, to check for alternative conditions, or to provide a catch-all
clause for when the condition specified in the if statement doesn't apply.
